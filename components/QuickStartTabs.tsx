...
const tabs: { key: Language; label: string; icon: JSX.Element }[] = [
  {
    key: 'javascript',
    label: 'JavaScript',
    icon: (
...
export default function QuickStartTabs({
  ...
  snippets: Record<Language, SnippetData>
}) {
  ...
  return (
    <div className="rounded-xl border border-border overflow-hidden text-sm w-full text-left">
      {/* Toolbar */}
      <div className="flex items-center justify-between px-3 py-2 bg-card border-b border-border">
        ...
        <button onClick={copy} title="Copiar" className="text-muted hover:text-white transition-colors p-1">
          {copied ? (
      ...
      {/* Code */}
      <div
        className="quickstart-code [&>pre]:p-5 [&>pre]:overflow-x-auto [&>pre]:!bg-[#0d1117] [&>pre]:font-mono [&>pre]:text-[13px] [&>pre]:leading-[1.8]"
        dangerouslySetInnerHTML={{ __html: snippets[active].html }}
      />
    </div>
  )
}
